...
import { empty } from 'rxjs/observable/empty';
import { merge } from 'rxjs/observable/merge';
import { delay, map, mapTo, switchMap, tap, mergeMap, takeUntil, filter, finalize } from 'rxjs/operators';
...
const catImage: HTMLImageElement = <HTMLImageElement>document.getElementById('cat');
// Stop polling
let stopPolling$ = fromEvent(stopButton, 'click').pipe(
...
function watchForData(category: RequestCategory) {
    ...
    return startPolling(category, 5000).pipe(
      ...
      takeUntil(
        ...
        ).pipe(tap(_ => console.log('Stopped'))) 
      ),
...
// Start Polling
merge(fromEvent(startButton, 'click'),
